...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{Logos/Diagrama de base de datos vectorial.jpeg}
...
Las pruebas de caja negra validaron el comportamiento del sistema desde la perspectiva del usuario final, evaluando la interacción conversacional sin conocer la lógica interna del código. Para este propósito, se estructuró un conjunto de pruebas diseñadas para evaluar tanto casos ideales (caminos felices) como situaciones límite (preguntas ambiguas o fuera de contexto). Se formularon 50 consultas académicas aleatorias que debían ser respondidas exclusivamente utilizando la información inyectada en la base de datos vectorial ChromaDB.

Durante las pruebas, se prestó especial atención a la capacidad de comprensión del Asistente Virtual Multimodal frente a modismos locales y formulaciones gramaticales informales por parte de los estudiantes. Se comprobó que, ante preguntas fuera del dominio (ej. receta médica), el sistema respondiera correctamente "No dispongo de información reglamentaria para responder esto", mitigando eficazmente el riesgo de alucinaciones. 

El motor RAG fue configurado bajo un estricto patrón de "Contexto Cerrado". Si la métrica de similitud del coseno es inferior al umbral mínimo establecido, el prompt del sistema instruye al LLM a declinar la respuesta. Este proceso de validación externa garantizó que el sistema mantenga un alto grado de rigor y fiabilidad institucional.
...
\begin{figure}[htbp]
  {\centering
  \includegraphics[width=0.85\textwidth]{Extras/PREGUNTA11.png}
  \caption{Preferencia futura de uso del asistente virtual.}\par}
