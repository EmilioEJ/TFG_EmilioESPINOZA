...
\begin{table}[H]
  ...
  \begin{tabular}{|p{1.5cm}|p{3.5cm}|p{8.5cm}|}
    \hline
    \rowcolor[HTML]{EFEFEF}
    \textbf{Código} & \textbf{Nombre} & \textbf{Descripción Técnica} \\ \hline
    \textbf{RF-01} & Interacción Visual no Biométrica & El sistema debe detectar la presencia del usuario frente a la cámara web analizando exclusivamente cajas delimitadoras (bounding boxes), sin aplicar biometría facial ni reconocimiento de identidad. \\ \hline
    \textbf{RF-02} & Captura y Transcripción de Voz (STT) & El sistema debe capturar el audio del micrófono del usuario de forma asíncrona y transcribirlo a texto mediante un modelo Whisper-v3 vía API externa, manejando jerga académica. \\ \hline
    \textbf{RF-03} & Motor RAG Multilingüe & El sistema debe procesar la consulta utilizando embeddings en español y recuperar los fragmentos documentales más relevantes de una base de datos vectorial local (ChromaDB). \\ \hline
    \textbf{RF-04} & Generación de Respuesta (LLM) & El sistema debe enviar la instrucción base (prompt) más el contexto recuperado al LLM para generar una respuesta natural y libre de alucinaciones. \\ \hline
    \textbf{RF-05} & Síntesis de Texto a Voz (TTS) & El sistema debe convertir el texto de la respuesta en audio mediante un flujo de streaming segmentado (HTTP Chunking), reduciendo el TTFB. \\ \hline
    \textbf{RF-06} & Renderizado y Lip-Sync 3D & El sistema debe renderizar un modelo 3D (VRM) en WebGL y sincronizar la apertura de boca del avatar con los fonemas extraídos del audio sintetizado. \\ \hline
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \small
  \renewcommand{\arraystretch}{1.2}
  \begin{tabular}{|p{1.5cm}|p{2.5cm}|p{6cm}|p{3cm}|}
    \hline
    \rowcolor[HTML]{EFEFEF}
    \textbf{Código} & \textbf{Atributo} & \textbf{Descripción} & \textbf{Métrica / KPI} \\ \hline
    \textbf{RNF-01} & Seguridad y Privacidad & El procesamiento visual debe ser efímero. Prohibido el almacenamiento en disco de las imágenes capturadas. & 0 fotogramas almacenados en persistencia. \\ \hline
    \textbf{RNF-02} & Baja Latencia & El tiempo de respuesta de la síntesis de voz (TTFB) debe permitir una interacción conversacional natural. & TTFB $\le$ 800 milisegundos. \\ \hline
    \textbf{RNF-03} & Precisión Documental & El motor de búsqueda semántica debe garantizar la recuperación asertiva de reglamentos complejos. & \textit{Hit Rate} (Top-3) $\ge$ 90,0\%. \\ \hline
    \textbf{RNF-04} & Aislamiento & La arquitectura debe operar de forma agnóstica sin requerir conexión a bases de datos transaccionales. & 100\% desacoplado del SGA institucional. \\ \hline
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.15\textwidth]{Logos/diagrama_contexto.pdf}}
...
\begin{figure}[htbp]
  \centering
  \makebox[\textwidth][c]{\includegraphics[width=1.15\textwidth]{Logos/diagrama_clases.pdf}}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\linewidth,height=0.95\textheight,keepaspectratio]{Logos/diagrama_estados.pdf}
