...
La adopción de tecnologías emergentes de Inteligencia Artificial (IA) en la educación superior se articula a través de tres niveles interdependientes de análisis: macroeconómico y nacional, meso o interinstitucional metropolitano, y micro o específicamente universitario.

En el \textbf{contexto Macro (Ecuador)}, según el \textit{Índice Latinoamericano de Inteligencia Artificial (2024-2025)} \cite{ilia2024ecuador}, el país es catalogado en una etapa de ``adopción tardía'', pero con un crecimiento acelerado en el desarrollo de talento tecnológico e infraestructuras digitales. En la educación superior ecuatoriana, un estudio reciente demostró que el 87,3\% de los estudiantes universitarios reporta haber utilizado herramientas basadas en IA para fines formativos \cite{wiseguyreports2025}, cifra que alcanza un 94,2\% entre alumnos de facultades técnicas y científicas como la Ingeniería en Tecnologías de la Información. Los asistentes generativos conversacionales son utilizados por el 82,7\% de la población estudiantil; sin embargo, esta penetración ocurre predominantemente ``desde abajo'', de manera autodidacta y paralela a las plataformas oficiales, evidenciando la necesidad imperativa de contar con canales y estrategias institucionales formalmente gobernadas \cite{unescoin2024}.

En el \textbf{contexto Meso (Universidades de Quito)}, diversas instituciones del distrito metropolitano lideran la adopción aplicada de inteligencia artificial en la automatización académica. La Pontificia Universidad Católica del Ecuador (PUCE) desarrolló \textit{Aurel\_AI} para transformar digitalmente su mesa de ayuda universitaria, así como \textit{LegalBot-EC}, un modelo de lenguaje local especializado en normativas legales ecuatorianas \cite{puce2024aurel}. Por su parte, la Universidad San Francisco de Quito (USFQ) y la Universidad Central del Ecuador (UCE) han desplegado modelos y motores locales para procesamiento analítico y soporte interno, preservando la privacidad de la información académica y demostrando la viabilidad técnica de sistemas autónomos en la región \cite{ilia2024ecuador}.

En el \textbf{contexto Micro (Universidad Tecnológica Indoamérica - UTI)}, la institución —con sedes operativas consolidadas en Quito y Ambato— ha mantenido una oferta formativa orientada a la innovación digital. De acuerdo con los informes institucionales de Rendición de Cuentas \cite{uti2025rc}, la universidad registra una matrícula global que supera los 8.358 estudiantes (7.839 en programas de grado). En específico, la carrera de Ingeniería en Tecnologías de la Información reporta una población matriculada estable de aproximadamente 380 estudiantes activos en sus dos sedes, evidenciando un notable crecimiento en admisiones de primer nivel en el periodo académico reciente (68 nuevos alumnos: 47 en la sede Quito y 21 en la sede Ambato) \cite{uti2025rc}. Históricamente, la universidad ha superado la marca de 6.300 estudiantes regulares, generando un volumen masivo de consultas operativas que saturan los esquemas tradicionales de secretaría.

La Tabla~\ref{tab:contexto_macro_meso_micro} sintetiza esta estructura analítica y los indicadores críticos de adopción tecnológica en cada dimensión.

\begin{table}[h]
  \caption{Estructura Analítica del Contexto Macro, Meso y Micro de Adopción de IA.}
  \label{tab:contexto_macro_meso_micro}
\centering
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{3.8cm}|>{\raggedright\arraybackslash}p{3.2cm}|>{\raggedright\arraybackslash}p{3.3cm}|}
\hline
\rowcolor[HTML]{EFEFEF}
\textbf{Nivel de Contexto} & \textbf{Área Geográfica / Institución} & \textbf{Indicador de Adopción IA} & \textbf{Enfoque Tecnológico y Reto} \\ \hline
\textbf{Macro} (Nacional) & República del Ecuador (Sistema Educativo Superior) & 87,3\% estudiantes usa IA (94,2\% en TI) \cite{ilia2024ecuador} & Adopción autodidacta ``desde abajo''; necesidad de gobernanza formal \\ \hline
\textbf{Meso} (Metropolitano) & Universidades de Quito (PUCE, USFQ, UCE) & Proyectos locales: \textit{Aurel\_AI}, \textit{LegalBot-EC} \cite{puce2024aurel} & Modelos locales, privacidad de datos y automatización de soporte \\ \hline
\textbf{Micro} (Institucional) & Universidad Tecnológica Indoamérica (UTI - Sede Quito/Ambato) & Matrícula de $>$8.358 alumnos ($\sim$380 en carrera TI) \cite{uti2025rc} & Evolucionar el portal SGA estático hacia un Asistente Virtual 3D proactivo \\ \hline
\end{tabular}
...
El desarrollo del presente Asistente Virtual Multimodal no constituye un esfuerzo aislado, sino que se inscribe orgánicamente dentro de una sólida trayectoria de innovación e investigación aplicada en la Universidad Tecnológica Indoamérica. A través de sus centros de investigación —como el Centro de Investigación en Mecatrónica y Sistemas Interactivos (MIST) y el Centro de Investigación en Empresa, Sociedad y Tecnología (ESTec)— la universidad ha ejecutado múltiples proyectos emblemáticos de inteligencia artificial y computación inmersiva que constituyen antecedentes científicos directos para este trabajo:

\begin{itemize}
    \item \textbf{Detección Oncológica mediante Visión Artificial e IA (Proyecto MIST-CEDIA):} En alianza con la Corporación Ecuatoriana para el Desarrollo de la Investigación y la Academia (CEDIA), la UTI lidera un proyecto I+D+i de modelamiento de procesos biológicos e imágenes médicas para el análisis y detección temprana del cáncer de mama \cite{uti2025cedia}. Este antecedente consolida el dominio computacional de la institución en visión artificial y algoritmos de detección visual por cajas delimitadoras (\textit{bounding boxes}).
    \item \textbf{Entornos Inmersivos 3D y Pensamiento Crítico en TI:} En la propia carrera de Ingeniería en Tecnologías de la Información, investigadores desarrollaron entornos de simulación en motores gráficos avanzados (\textit{Unreal Engine 5}), logrando incrementar el índice de pensamiento crítico de los estudiantes de un promedio de 3.3/10 a 4.1/10 \cite{uti2024unreal}. Ello demuestra empíricamente que la interacción con interfaces tridimensionales dinámicas mejora el rendimiento y la participación del estudiante.
    \item \textbf{Análisis de Sentimientos mediante PNL:} A nivel de procesamiento semántico, investigadores de la UTI han aplicado técnicas avanzadas de Procesamiento de Lenguaje Natural (PLN) y análisis de Big Data para medir sentimientos y percepciones institucionales en medios digitales \cite{uti2024pln}.
    \item \textbf{Evaluación Formativa Asistida por IA en Posgrados y Capacitación Docente:} La institución ha impulsado proyectos de evaluación continua asistida por IA para mitigar sesgos en programas académicos, al tiempo que ha capacitado a su profesorado en inteligencia artificial con expertos de talla mundial para posicionarse como un nodo de innovación iberoamericano \cite{uti2025rc}.
\end{itemize}

La Tabla~\ref{tab:proyectos_internos_uti} sistematiza los proyectos internos previos que sirven como cimiento metodológico y computacional para la integración del avatar 3D y el motor RAG conversacional en la carrera de TI.

\begin{table}[h]
  \caption{Antecedentes Institucionales de Investigación en IA y Sistemas Multimodales (UTI).}
  \label{tab:proyectos_internos_uti}
\centering
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{3.0cm}|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3.5cm}|}
\hline
\rowcolor[HTML]{EFEFEF}
\textbf{Proyecto Institucional} & \textbf{Unidad / Centro UTI} & \textbf{Tecnología Aplicada} & \textbf{Aporte al Asistente de Tesis} \\ \hline
\textbf{Proyecto MIST-CEDIA} \cite{uti2025cedia} & Centro MIST / CEDIA & Visión artificial, imágenes y \textit{Deep Learning} & Arquitectura de detección visual en tiempo real (\textit{bounding boxes}) \\ \hline
\textbf{Simulaciones 3D en TI} \cite{uti2024unreal} & Facultad TI & Motor gráfico 3D (\textit{Unreal Engine 5}) & Validación del impacto de interfaces 3D en el compromiso estudiantil \\ \hline
\textbf{Análisis Sentimientos PNL} \cite{uti2024pln} & Facultad TI / ESTec & Procesamiento de Lenguaje Natural y Big Data & Comprensión semántica del lenguaje y consultas informales \\ \hline
\textbf{Evaluación Formativa IA} \cite{uti2025rc} & Posgrados / Docencia & IA generativa en aulas virtuales & Integración curricular sin sesgos y estandarización del servicio \\ \hline
\end{tabular}
...
\setlength{\LTcapwidth}{18.5cm}
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.18}
\begin{xltabular}{\linewidth}{|>{\raggedright\arraybackslash\hsize=0.8\hsize}X|>{\raggedright\arraybackslash\hsize=0.9\hsize}X|>{\raggedright\arraybackslash\hsize=0.9\hsize}X|>{\raggedright\arraybackslash\hsize=0.9\hsize}X|>{\raggedright\arraybackslash\hsize=1.5\hsize}X|}
...
\subsection{Utilidad}
La utilidad principal del prototipo radica en proporcionar un mecanismo complementario de acceso a información académica mediante lenguaje natural, reduciendo la necesidad de navegar manualmente por documentos extensos o múltiples secciones institucionales. La arquitectura RAG permite vincular las respuestas del asistente con una base documental previamente seleccionada, mientras que la interfaz multimodal busca ofrecer una interacción más natural mediante texto, voz y avatar 3D.

\subsection{Beneficiarios}
Los beneficiarios directos son los estudiantes de la carrera de Tecnologías de la Información, quienes dispondrían de un canal adicional para consultar información académica general. De manera indirecta, la disponibilidad de un mecanismo automatizado podría contribuir a reducir determinadas consultas repetitivas dirigidas al personal administrativo; sin embargo, la cuantificación de esta reducción requiere un seguimiento posterior en condiciones reales de operación.

\subsection{Factibilidad e Impacto Potencial}
La ejecución de este proyecto es factible tanto a nivel técnico como operativo. Actualmente existen frameworks de inteligencia artificial, APIs de síntesis de voz, librerías de visión artificial y plataformas de creación de avatares que son altamente accesibles para integrarse en un prototipo funcional de software. Además, el proyecto posiciona a la Universidad Indoamérica a la vanguardia en la adopción de interfaces humano-computadora avanzadas.

Para ilustrar el impacto potencial del proyecto en la carrera de Ingeniería en Tecnologías de la Información, se ha diseñado la matriz comparativa de la Tabla~\ref{tab:cuantificacion_impacto}. En ella se contrastan los indicadores críticos de la situación actual frente a la proyección técnica del Asistente Virtual Inteligente.

\begin{table}[htbp]
  \caption{Matriz de Indicadores de Impacto Potencial del Proyecto.}
  \label{tab:cuantificacion_impacto}
\centering
\small
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{|>{\raggedright\arraybackslash}p{2.8cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3.5cm}|>{\raggedright\arraybackslash}p{3.2cm}|}
\hline
\rowcolor[HTML]{EFEFEF}
\textbf{Indicador} & \textbf{Situación Actual} & \textbf{Proyección del Prototipo} & \textbf{Interpretación} \\ \hline
\textbf{Población potencial beneficiaria} & Aproximadamente 380 estudiantes de TI & Acceso al prototipo para consulta de mallas y normativas & Cobertura potencial de la población objetivo \\ \hline
\textbf{Consulta de información} & Navegación manual por sistemas y documentos PDF & Consulta mediante lenguaje natural & Simplificación del mecanismo de consulta \\ \hline
\textbf{Recuperación de información} & Búsqueda manual o por palabras clave & \textit{Hit Rate} Top-3 de 92,0\% & El fragmento relevante se recuperó entre los tres primeros resultados en el 92\% de las pruebas \\ \hline
\textbf{Tiempo tecnológico de respuesta} & No aplica directamente & Métricas de latencia medidas en el prototipo & Permite evaluar la viabilidad de interacción conversacional \\ \hline
\textbf{Disponibilidad potencial} & Dependiente del horario de los canales administrativos & El software puede operar fuera del horario administrativo & Capacidad potencial de ampliar el horario de consulta \\ \hline
\end{tabular}
...
\begin{itemize}
    \item Analizar los procesos actuales de gestión y consulta de documentación académica en la institución mediante revisión documental y observación directa, para determinar los requerimientos funcionales, no funcionales y la base de conocimiento conversacional del sistema.
    \item Diseñar la arquitectura de software del asistente mediante un enfoque modular cliente-servidor, definiendo la integración del módulo conversacional, el motor RAG, la base de conocimiento académica, los servicios de voz y el entorno gráfico del avatar 3D.
    \item Implementar el prototipo del asistente virtual integrando los modelos de inteligencia artificial locales, la interfaz antropomórfica web y los mecanismos de interacción visual no biométrica, para obtener una prueba de concepto interactiva funcional.
    \item Evaluar el desempeño, rendimiento y usabilidad del prototipo mediante pruebas de interacción con una muestra de estudiantes y métricas computacionales (latencia, uso de recursos), para validar empíricamente la viabilidad técnica de la solución propuesta.
\end{itemize}
...
En el paradigma RAG, los documentos son fragmentados (\textit{chunking}) y transformados a vectores multidimensionales (\textit{embeddings}). Durante la búsqueda, la precisión semántica de la consulta del usuario se evalúa matemáticamente mediante la \textbf{Similitud del Coseno} (Figura~\ref{fig:coseno_rag}), que mide el ángulo entre el vector de la consulta ($\vec{q}$) y el vector del documento ($\vec{d_i}$) en el espacio latente. Una mayor similitud semántica se traduce en un menor ángulo $\theta$.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}
    % Ejes
    \draw[->, thick] (0,0) -- (8,0) node[right] {$Dim_1$};
    \draw[->, thick] (0,0) -- (0,6) node[above] {$Dim_2$};
    
    % Vector Documento
    \draw[->, thick, blue] (0,0) -- (6,1.5) node[right, yshift=-5pt] {Vector Documento ($\vec{d_i}$)};
    
    % Vector Consulta
    \draw[->, thick, red] (0,0) -- (3,5) node[above left] {Vector Consulta ($\vec{q}$)};
    
    % Arco del ángulo
    \draw[bend right, <->, thick, green!60!black] (1.5,2.5) to (2.8, 0.7);
    
    % Etiqueta del ángulo manual para evitar solapamiento
    \node[green!60!black, below] at (2.5, 0.5) {$\theta$ (Similitud Coseno)};
    
    % Texto explicativo
    \node[align=left, anchor=west] at (4.5, 5.0) {
        \small Similitud Coseno ($\theta$) = $\frac{q \cdot d_i}{||q|| \cdot ||d_i||}$ \\[1.5ex]
...
La arquitectura de los asistentes virtuales modernos sigue un patrón modular y distribuido que separa la interacción con el usuario (cliente) del procesamiento cognitivo intensivo (servidor). En la capa de cliente o \textit{frontend}, se gestiona la captura en tiempo real de los flujos de audio y la detección visual ligera de presencia a través de tecnologías web estándar. Esta capa es también la responsable de renderizar gráficamente el avatar 3D para proveer retroalimentación visual al usuario. 

Por su parte, la capa de servidor o \textit{backend} agrupa los servicios cognitivos: motores de transcripción (\textit{Speech-to-Text}), la orquestación del procesamiento de lenguaje natural integrado con Generación Aumentada por Recuperación (RAG), la consulta a la base de datos vectorial y la síntesis de voz (\textit{Text-to-Speech}) \cite{pabon2025naia}. Esta división estratégica asegura que la latencia en la comunicación sea mínima, delegando las cargas computacionales pesadas a servidores especializados o contenedores aislados. De esta forma, se mantiene una experiencia de usuario conversacional fluida e ininterrumpida en el punto de interacción, consolidando la funcionalidad de soporte institucional continuo y automatizado.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{Logos/diagrama_arquitectura_software.jpeg}
  \caption{Arquitectura general de software para el Asistente Virtual Multimodal.}
  \label{fig:arquitectura_software}
\end{figure}

\subsection{Aceptación Tecnológica en la Educación Superior}
